  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-transparent">
      <div className="relative mx-auto flex max-w-7xl items-center justify-between gap-3 px-6 py-4">
        ...
        <nav className="hidden md:flex items-center gap-1 rounded-full border border-white/15 bg-white/15 px-2 py-1.5 backdrop-blur md:absolute md:top-1/2 md:-translate-y-1/2" style={{ left: "calc(50% + 8px)", transform: "translateX(-50%) translateY(-50%)" }}>
          {navItems.map((n) => (
            <Link
              key={n.to}
              to={n.to}
